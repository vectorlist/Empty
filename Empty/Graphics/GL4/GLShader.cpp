...
void GLShader::InitFromFile(const char* vs, const char* fs)
{
	auto vsCode = this->FileToCode(vs);
	auto fsCode = this->FileToCode(fs);

	this->InitFromCode(vsCode.c_str(), fsCode.c_str());
}

void GLShader::InitFromCode(const char* vs, const char* fs)
{
